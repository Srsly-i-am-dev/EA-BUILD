...
namespace cAlgo.Robots
{
    ...
    public class fib_regime_7 : Robot
    {
        // -----------------------------
        ...
        [Parameter("Min Impulse ATR", DefaultValue = 0.4)]
        ...
        // -----------------------------
        // NEW: SL/TP Geometry Guardrails (NO core change)
        // -----------------------------
        [Parameter("Min SL (pips, 0=off)", DefaultValue = 10)]
        public double MinSLPips { get; set; }

        [Parameter("Max RR (0=off)", DefaultValue = 4)]
        ...
        [Parameter("Risk Mode", DefaultValue = RiskMode.AtrFromEntry)]
        ...
        [Parameter("ATR SL Mult (AtrFromEntry)", DefaultValue = 1.5)]
        public double AtrSLMult { get; set; }

        [Parameter("ATR TP Mult (AtrFromEntry)", DefaultValue = 2.5)]
        ...
        [Parameter("Min ATR14/ATR50", DefaultValue = 0.60)]
        ...
        [Parameter("Min EMA Slope (ATR-norm)", DefaultValue = 0.15)]
        public double MinEmaSlopeNorm { get; set; }

        [Parameter("Min Impulse Body Ratio", DefaultValue = 0.40)]
        ...
        // Build-proof marker store (avoids Position.Id type issues)
        ...
        protected override void OnBar()
        {
            ...
            if (bullishTrend && IsBullishImpulse(i))
            {
                ...
                ActivateSetup(
                    ...
                    Math.Min(f50, f618),
                    Math.Max(f50, f618),
                    Bars.LowPrices[i] - atrFast.Result[i] * 0.8,
                    high + atrFast.Result[i] * 1.5,
                    i
                );
            }
        ...
        // -----------------------------
        // SL/TP Guardrails (MinSLPips + MaxRR)
        // -----------------------------
        private void ApplyMinSLAndMaxRR(TradeType type, double entryRef, ref double slPrice, ref double tpPrice)
        {
            double slPips = Math.Abs(entryRef - slPrice) / Symbol.PipSize;
            double tpPips = Math.Abs(tpPrice - entryRef) / Symbol.PipSize;

            // A) Minimum SL floor
            if (MinSLPips > 0 && slPips < MinSLPips)
                slPips = MinSLPips;

            if (MinSLPips > 0)
            {
                slPrice = (type == TradeType.Buy)
                    ? entryRef - slPips * Symbol.PipSize
                    : entryRef + slPips * Symbol.PipSize;
            }

            // Recompute TP pips after SL change (TP unchanged so far)
            tpPips = Math.Abs(tpPrice - entryRef) / Symbol.PipSize;

            // B) Max RR cap
            if (MaxRR > 0)
            {
                double maxTpPips = slPips * MaxRR;
                if (tpPips > maxTpPips)
                    tpPips = maxTpPips;

                tpPrice = (type == TradeType.Buy)
                    ? entryRef + tpPips * Symbol.PipSize
                    : entryRef - tpPips * Symbol.PipSize;
            }
        ...
        private void ExecuteTrade(TradeType type, double slPriceBase, double tpPriceBase)
        {
            ...
            // ✅ Guardrails
            ApplyMinSLAndMaxRR(type, entryRef, ref slPrice, ref tpPrice);
        ...
        private void ManageInsurance()
        {
            ...
            if (main == null)
            {
                if (hedge != null) ClosePosition(hedge);
                return;
            }
