...
class SMP1UpdateCool(cBot):
    ...
    MaxLots = Parameter(default=0.5, description="Max position size in lots")
    MaxSLPips = Parameter(default=150.0, description="Hard SL cap in pips (0=off)")
    CustomSL = Parameter(default=0.0, description="Custom SL pips (0=swing-based)")
    CustomTP = Parameter(default=0.0, description="Custom TP pips (0=swing-based)")
    CooldownAtrMultiplier = Parameter(default=1.5, description="ATR mult for cooldown lift (0.5–5.0)")
    ...
    def on_start(self):
        ...
        self._setup_swing_high = float("-inf")
        self._setup_swing_low = float("inf")

        self._in_cooldown = False
        self._cooldown_swing_high = float("-inf")
        self._cooldown_swing_low = float("inf")
        ...
        if self.UseMlFilter:
            try:
                ...
                self.Print(f"[SMP1UpdateCool] ML model loaded from {self.MlModelPath}")
            except Exception as e:
                self.Print(f"[SMP1UpdateCool] ML load failed: {e}")
    ...
    def on_bar(self):
        ...
        if self._setup_active:
            return
        if self._in_cooldown and not self._try_lift_cooldown(i):
            return
        ...
        high = max(self.Bars.HighPrices[x] for x in range(start, i + 1))
        low = min(self.Bars.LowPrices[x] for x in range(start, i + 1))
        ...
        if bullish_trend and self._is_bullish_impulse(i, atr_val):
            ...
            tp = high + atr_val * 1.5
            self._setup_swing_high = high
            self._setup_swing_low = low
    ...
    def on_tick(self):
        ...
        if self.Positions.Find(self.BOT_LABEL, self.SymbolName) is not None:
            return
        if len(self.Bars) - 2 > self._setup_bar_index + 8:
            self._reset_setup()
            return
    ...
    def _on_position_closed(self, args):
        ...
        self._cooldown_swing_high = self._setup_swing_high
        self._cooldown_swing_low = self._setup_swing_low

    def _try_lift_cooldown(self, i):
        atr_val = self._atr.Result[i]
        if atr_val <= 0:
            return False
        start = max(0, i - self.SwingLookback)
        curr_high = max(self.Bars.HighPrices[x] for x in range(start, i + 1))
        curr_low = min(self.Bars.LowPrices[x] for x in range(start, i + 1))
        margin = atr_val * self.CooldownAtrMultiplier
        if curr_high > self._cooldown_swing_high + margin or curr_low < self._cooldown_swing_low - margin:
            self._in_cooldown = False
            return True
        return False
